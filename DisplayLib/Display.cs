...
namespace DisplayLib
{
    public class Display
    {
        ...
        private IEnumerable<Display.Orientation> orientation;

        private Display()
        {
            this.orientation = this.GetAllRotations().Select(e => (Display.Orientation)e);
        }
        ...
        public bool Rotate(uint DisplayNumber, Orientation Orientation)
        {
            if (DisplayNumber == 0)
        ...
        private IEnumerable<int> GetAllRotations()
        {
            ...
            try
            {
                ...
                while (++i <= 64)
                {
                    if (!NativeMethods.EnumDisplayDevices(null, i - 1, ref d, 0))
                        throw new ArgumentOutOfRangeException("DisplayNumber", i, "Number is greater than connected displays.");

                    if (0 != NativeMethods.EnumDisplaySettings(d.DeviceName, NativeMethods.ENUM_CURRENT_SETTINGS, ref dm))
                        r.Add(dm.dmDisplayOrientation);
                }
            }
            catch
            {
        ...
        public void ResetAllRotations()
        {
            try
            {
                ...
                while (++i <= 64)
                {
                    this.Rotate(i, this.orientation.ToArray()[(i - 1)]);
                }
            }
            catch
            {
